...
use crate::models::{Archive, Index, Entry};
...
pub fn parse_archive<P: AsRef<Path>>(path: P) -> anyhow::Result<(Archive, Index, Vec<Entry>)> {
    ...
    let archive = archive::open(path_ref).with_context(|| format!("opening archive {}", path_ref.display()))?;

    let mut f = File::open(path_ref).with_context(|| format!("opening file {} for index read", path_ref.display()))?;
