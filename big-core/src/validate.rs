...
pub fn validate_archive<P: AsRef<std::path::Path>>(path: P) -> anyhow::Result<ValidationResult> {
    ...
    if index_offset as u128 >= size as u128 {
        push_error(
            &mut result,
            "INDEX_OOB",
            format!("Index offset {} is outside file (size {})", index_offset, size),
        );
        return Ok(result);
    }
    ...
    if let Err(e) = (|| -> anyhow::Result<()> {
        ...
        if entries.len() != index_count as usize {
            push_warning(
                ...
                format!(
                    "Index count header {} differs from parsed entries {}",
                    index_count,
                    entries.len()
                ),
            );
        ...
        if let Ok(cur) = f.stream_position()
            && let Some(min_off) = entries.iter().map(|e| e.offset).min()
            && min_off < cur
        {
            push_error(
                &mut result,
                "PAYLOAD_BEFORE_INDEX_END",
                format!("First payload offset {} is before index end {}", min_off, cur),
            );
        }
