...
fn iterator_yields_entries_in_order() {
    let entries = vec![
        Entry { name: "a.txt".into(), offset: 0, length: 10, compressed: false, r#type: Some("text".into()) },
        Entry { name: "b.png".into(), offset: 10, length: 200, compressed: true, r#type: Some("image".into()) },
    ];
...
fn resolve_metadata_returns_type() {
    let entry = Entry { name: "x.bin".into(), offset: 0, length: 4, compressed: false, r#type: Some("binary".into()) };
